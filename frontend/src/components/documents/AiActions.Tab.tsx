...
const AiActionsTab = () => {
  ...
  const handleExplainConcept = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setModalContent(response.explaination);
      setIsModalOpen(true);
      setConcept("");
    } catch (error) {
      console.log(error);
            ...
            <form onSubmit={handleExplainConcept}>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 h-11 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-50 text-sm font-medium transition-all duration-200 focus:outline-none focus:border-emerald-500 focus:shadow-lg focus:bg-white focus:shadow-purple-500/10"
                  disabled={loadingAction === "explain"}
                />
                <button
                  className="shrink-0 h-11 px-5 bg-linear-to-br from-emerald-600 to-emerald-500 hover:from-emerald-600 hover:to-emerald-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:cursor-not-allowed scale-[0.98]"
                  type="submit"
